...
<?php
	...
	$SQL = "SELECT gt.sName, ut.uNick, ut.uEmail ,pt.* FROM UsersTBL ut, ReceiveMsgTBL pt, GradeTBL gt WHERE (ut.userID = pt.fromUser) AND (ut.gradeID = gt.gradeID) AND (pt.messageID={$messageID})";
...
<?php
	...
	// 즉 바로 이전 글
	$result_pid = qsSysSelectSQL("SELECT messageID FROM ReceiveMsgTBL WHERE (messageID > $messageID) AND (toUser='{$logID}') LIMIT 1");
	$prev_id = $result_pid->fetch();

	if ($prev_id['messageID']) // 이전 글이 있을 경우
	{
		echo "<a href=main.php?rightView=messageDetail.php&MESSAGEID={$prev_id['messageID']}>[이전]</a>";
	}
	else
	{
	...
	$result_nid = qsSysSelectSQL("SELECT messageID FROM ReceiveMsgTBL WHERE (messageID < $messageID) AND (toUser='{$logID}')ORDER BY messageID DESC LIMIT 1");
	$next_id = $result_nid->fetch();

	if ($next_id['messageID'])
	{
		echo "<a href=main.php?rightView=messageDetail.php&MESSAGEID={$next_id['messageID']}>[다음]</a>";
	}
	else
	{
...
<?php
	$result= qsSysExecuteSQL("UPDATE ReceiveMsgTBL SET isRead =isRead +1 WHERE messageID =$messageID");
