...
//-----------------------------------------------------
// 총 게시물의 숫자.
$SQL = "SELECT count(mt.fromUser) FROM ReceiveMsgTBL mt, UsersTBL ut WHERE (ut.userID   = mt.fromUser)";
$SQL = $SQL .  "AND (mt.toUser = '{$_SESSION['LOGINID']}')  " . $sWhere;
...
//
$SQL ="SELECT  ut.uNick , ut.uEmail, mt.* FROM UsersTBL ut, ReceiveMsgTBL mt WHERE (ut.userID   = mt.fromUser) ";
$SQL = $SQL . "AND (mt.toUser = '{$_SESSION['LOGINID']}')  " . $sWhere ." ORDER BY mt.wTime  DESC LIMIT $no,$viewCount";
$result = qsSysSelectSQL($SQL);
//-----------------------------------------------------
// 총 게시물 수 를 구한다.
...
<tr height=25 bgcolor=white>
	...
	<!-- 제목 -->
	<td>&nbsp;
		<a href="main.php?rightView=./service/message/messageDetail.php&MESSAGEID=<?php echo $row['messageID']?>">
