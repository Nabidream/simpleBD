...
//-----------------------------------------------------
// 총 게시물의 숫자.
$SQL = "SELECT count(mt.userID) FROM ProjectTBL mt, UsersTBL ut WHERE (ut.userID   = mt.userID)";
$SQL = $SQL .  "AND (mt.userID = '{$_SESSION['LOGINID']}')  " . $sWhere;
...
//-----------------------------------------------------
$SQL ="SELECT ut.uNick , ut.uEmail, mt.* FROM UsersTBL ut, ProjectTBL mt WHERE (ut.userID   = mt.userID) ";
$SQL = $SQL . "AND (mt.userID = '{$_SESSION['LOGINID']}')  " . $sWhere ." ORDER BY mt.wTime  DESC LIMIT $no,$viewCount";
$result = qsSysSelectSQL($SQL);
//-----------------------------------------------------
...
<tr>
	<th colspan=4>프로젝트 록목</th>
</tr>
<tr>
	<th align=center><b>완 료</b></th>
	<th align=center><b>제 목</b></th>
	<th align=center><b>소유자</b></th>
	<th align=center><b>날 짜</b></th>
</tr>
<?php
...
<tr height=25 bgcolor=white>
	<!-- 완료율-->
	<td align=center>
		<?php echo $row['doneRate'];?>
	</td>
	<!-- 제목 -->
	<td>&nbsp;
		<a href="main.php?rightView=./service/project/projectDetail.php&PROJECTID=<?php echo $row['projectID']?>">
...
<button class="textButton" onclick="GoRightView('./service/project/projectWrite.php')">
	[글쓰기]</button>
<br>
<br>
