...
<?php
	include_once "./lib/qsFunc.php";
	// 먼저 쓴 글의 정보를 가져온다.
	$result=qsSysSelectSQL("SELECT * FROM ProjectTBL WHERE projectID=$projectID");
	$row= $result->fetch();
?>
<!-- 입력 부분 -->
	<tr>
		...
		<table class="mainTable" >
			<form>
			<tr><td width=60 align=left >완료율</td>
				<td align=left >
					<INPUT type=text name=title size=60 id="editRate" style="width:100%"
					value=<?php echo $row['doneRate']?>>
			</td></tr>
			<tr><td width=60 align=left >제 목</td>
				<td align=left >
			...
			<tr>
				...
				<!--	<button type="button" class="textButton" onclick="axPostModify(88,'editTitle','editContent')">[저장]</button>-->
					<button type="button" class="textButton" onclick="axProjectModify(<?php echo $projectID ?>,'editRate','editTitle','editContent');">[저장]</button>
					&nbsp;&nbsp;
